    ...
    on: {
        ...
        slideChange: function () {
            setTimeout(() => {
                ...
                activeMenu = menuLinks[pageSlider.realIndex]
                logicaLine();
            }, 0);
        },
        resize: function () {
...
function setScrollType() {
    for (let i = 0; i < pageSlider.slides.length; i++) {
        const slide = pageSlider.slides[i],
...
let overlay = document.querySelector('.overlay'),
    ...
    menuLine = document.querySelector('.menu__line'),
    activeMenu = menuLinks[pageSlider.realIndex];

let logicaLine = (el = activeMenu) => menuLine.style.top = `${el.offsetHeight + el.offsetTop + 6}px`;

menuLinks.forEach((link, i) => {
    link.addEventListener('mouseenter', function () { logicaLine(this) });
    link.addEventListener('click', function () {
        menuRemvoe();
        this.classList.add('active');
        activeMenu = this;
        console.log(body.offsetWidth);
        if (body.offsetWidth <= 920) header.classList.remove('header--active');
    });
});

let menuRemvoe = () => activeMenu.classList.remove('active');

menu.addEventListener('mouseleave', () => logicaLine());
...
burger.addEventListener('click', function () {
    ...
    header.classList.contains('header--active') ?
        this.ariaLabel = 'закрыть меню' : this.ariaLabel = 'открыть меню';
});

const mapСoordinates = { lat: 56.635189, lng: 59.282952 },
...
function initMap() {
    ...
    new google.maps.Marker({
        position: mapСoordinates,
        map: map,
        icon: "https://svgshare.com/i/Wtb.svg",
        scale: 1,
        fillOpacity: 0.1,
    });
}